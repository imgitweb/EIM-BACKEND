      ...
      if (!companyDetails) {
        ...
        if (!ownerId) {
          return res
            ...
            .json({
              error:
                "Company details not found and no userId available to create record",
            });
        }
        ...
        try {
          const created = await CompanyDetails.create({
            _id: id,
            ...createData,
          });
          // populate userId for consistency
          const populated = await CompanyDetails.findById(created._id).populate(
            "userId",
            "name email"
          );
          ...
          return res
            .status(500)
            .json({
              error: createErr.message || "Error creating company details",
            });
        }
      }
